...
**Linux.do 小秘书** 帮你解决这个问题——一个悬浮在页面角落的小助手，实时显示你的信任级别进度、积分余额、CDK 社区分数，内置主页筛选工具，支持数据变化追踪，再也不用频繁切换页面了。
...
### 个性化定制
- **小秘书图标** - 全新小秘书形象悬浮球，hover 时自动切换表情，支持三种尺寸选择
...
安装后访问 [linux.do](https://linux.do)，页面右侧会出现小秘书悬浮球。

| 操作 | 说明 |
|---|---|
| **点击悬浮球** | 展开仪表盘面板 |
| **悬停悬浮球** | 小秘书会切换表情（闭眼） |
| **长按悬浮球/顶栏按钮** | 在帖子页面快速返回第1楼 |
| **拖动悬浮球/标题栏** | 移动面板位置 |
| **切换标签页** | 信任级别 / 积分详情 / CDK 社区分数 / 主页筛选 / 偏好设置 |
| **点击主题图标** | 切换亮色/深色/跟随系统 |
| **点击刷新按钮** | 手动刷新数据 |
| **偏好设置** | 可调整语言、面板高度、透明度、字体大小、自动刷新频率等 |
| **拖拽排序标签** | 在偏好设置中，拖动标签项调整显示顺序，点击保存生效 |
| **检查更新** | 在标题栏点击更新图标，手动检查是否有新版本 |
| **主页筛选** | 按等级/分类/标签筛选帖子，支持保存预设 |
...
| 用户等级 | 显示内容 |
|---|---|
| **Lv.0 - Lv.1** | 浏览的话题、已读帖子、阅读时间、访问天数、送出赞、获赞、帖子数量 |
| **Lv.2+** | 访问天数、浏览的话题、已读帖子、送出赞、获赞、被举报次数、被禁言次数等 |

- ✅ 绿色表示已达标
- ❌ 红色表示未达标
- 数据来源标签显示当前使用的是 Connect 还是 Summary API
...
| **当前余额** | 你的 Credit 积分余额 |
| **今日剩余额度** | 每日可用的积分额度 |
| **预估明日涨分** | 基于 gamification_score 和基准值计算的预估涨分 |
| **近7日收支** | 最近一周的积分收入和支出明细 |
...
| 筛选条件 | 说明 |
|---|---|
| **按等级筛选** | 只显示指定信任等级用户发布的帖子（Lv.0 - Lv.4） |
| **按分类筛选** | 只显示指定分类下的帖子 |
| **按标签筛选** | 只显示带有指定标签的帖子 |
| **预设保存** | 保存常用的筛选条件组合，一键切换 |
| **拖拽排序** | 自定义预设的显示顺序 |
| **加载更多** | 当筛选结果较少时，自动加载更多帖子 |

### 长按快速返回

在帖子详情页面，**长按悬浮球或顶栏按钮**可以快速滚动到帖子的第1楼，方便阅读长帖子时快速回到顶部。

### CDK 社区分数

显示基于 cdk.linux.do 计算的社区信誉分数，首次使用需要完成 OAuth 授权。
...
| 设置项 | 说明 | 默认值 |
|---|---|---|
| **显示每日排名** | 启用后在信任级别页面显示今日排名和积分（会增加一次 Leaderboard API 请求） | 开启 |
| **自动展开面板** | 启用后，进入页面时自动展开仪表盘面板 | 关闭 |
| **涨分动画提示** | 启用后，积分涨分时显示绿色动画效果 | 开启 |
| **主页筛选工具** | 启用后在 linux.do 首页显示筛选工具栏 | 开启 |
| **自动刷新频率** | 后台自动刷新数据的间隔时间 | 30分钟 |
| **标签顺序** | 拖拽调整「信任级别」「积分详情」「CDK分数」标签页的显示顺序 | - |

### 外观设置

| 设置项 | 说明 | 默认值 |
|---|---|---|
| **显示模式** | 选择悬浮球模式或顶栏按钮模式 | 顶栏按钮 |
| **经典图标** | 启用后，悬浮球恢复为原版蓝色地球图标 | 关闭 |
| **图标大小** | 小秘书图标尺寸：小(58px) / 中(88px) / 大(128px) | 小 |
| **界面语言** | 中文 / English | 中文 |
| **面板高度** | 标准 / 加高 / 自适应 | 自适应 |
| **透明度** | 调节面板整体透明度（0-100%） | 100% |
| **字体大小** | 70%-130% 可调，支持一键恢复默认 | 100% |

> 💡 **提示**：
> - 使用前请确保已登录 linux.do 账号
> - 在偏好设置中可以自由拖拽标签顺序，打造专属你的面板布局
> - 关闭「显示每日排名」可以减少 API 请求，降低触发频率限制的风险
